...
class HumanoidEnv(PipelineEnv):
    ...
    def step(self, env_state: State, action: jnp.ndarray, rng: jnp.ndarray) -> State:
        ...
        # Function to generate reset state
        def generate_reset_state(rng):
        ...
        # Generate initial reset state
        rng, reset_rng = jax.random.split(rng)
        state_reset = generate_reset_state(reset_rng)

        # Check for NaNs in reset state and regenerate if necessary
        def has_nans(state):
            return jax.tree_util.tree_reduce(
                lambda a, b: a | jnp.any(jnp.isnan(b)), state, initializer=False
            )
        
        def regenerate_reset_state(carry):
            state, rng = carry
            rng, new_rng = jax.random.split(rng)
            new_state = generate_reset_state(new_rng)
            return (new_state, rng)

        state_reset, _ = jax.lax.while_loop(
            lambda x: has_nans(x[0]),
            regenerate_reset_state,
            (state_reset, reset_rng)
        )
        ...
        # setting done = True if nans in next state or action
        is_nan_step = jax.tree_util.tree_map(lambda x: jnp.any(jnp.isnan(x)), state_step)
        is_nan_action = jnp.any(jnp.isnan(action))
        
        any_nan = jax.tree_util.tree_reduce(jnp.logical_or, is_nan_step, initializer=False)
        any_nan = jnp.logical_or(any_nan, is_nan_action)
